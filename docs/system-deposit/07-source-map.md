...
| 文件 | 当前职责 | 已验证事实 | 重构关注点 |
| --- | --- | --- | --- |
| `backend/app/main.py` | FastAPI app、CORS、路由、价格录入、Excel 导入、模板下载 | `load_dotenv` 读取 `backend/.env`；多数路由直接写在入口文件 | 拆出 router 和 service；入口只注册 app |
...
| 文件 | 当前职责 | 已验证事实 | 重构关注点 |
| --- | --- | --- | --- |
| `frontend/src/views/ChartView.vue` | 图表主页面 | 超过 1200 行；直接读取 `/config.yaml`；多选 UI 只取第一个鱼种/市场请求 | 优先拆请求、时间段、图表配置 |
...
| 文件 | 当前事实 | 重构关注点 |
| --- | --- | --- |
| `backend/requirements.txt` | 缺少 `python-dotenv`、`python-multipart`、`httpx` | 补齐运行和测试依赖 |
| `frontend/package.json` | Vue 3、Vite 7、Element Plus、ECharts、Pinia | 构建可通过但 chunk 偏大 |
