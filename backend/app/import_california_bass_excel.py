...
EXCEL_PATH = "/Users/zcy/IdeaProjects/fish-price-platform/加州鲈塘口价历史数据.xlsx"
...
def parse_price(price_str: str) -> Optional[Decimal]:
    ...
    Args:
        price_str: 价格字符串，如 "10.5-11" 或 "16元/斤"
...
def get_source_code(source: str) -> str:
    ...
    for key, code in SOURCE_MAPPING.items():
        if key in source:
            return code
...
def validate_data(row: Dict[str, Any]) -> Tuple[bool, Optional[Dict[str, Any]], str]:
    ...
    if price > 100:
        return False, None, f"价格异常(超过100元/斤): {price_str}"
...
def main():
    ...
    print("\n确保基础数据存在...")
    ensure_fish_exists(conn, FISH_ID, "加州鲈", "鲈鱼", "CALIFORNIA_BASS")
    
    market_names = {
        201: ("广东佛山", "440600", "SOHU_BASS"),
        202: ("浙江湖州", "330500", "SOHU_BASS"),
    ...
    for data in valid_data:
        try:
            exists = check_existing_price(conn, FISH_ID, data["market_id"], data["date"], data["price"])
            
            if exists:
                delete_existing_price(conn, FISH_ID, data["market_id"], data["date"], data["price"])
                updated += 1
            else:
                inserted += 1
            
            insert_price(conn, FISH_ID, data["market_id"], data["price"], data["date"], data["source_url"])
